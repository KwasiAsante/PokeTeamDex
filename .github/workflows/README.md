...
> Runs `actions/setup-node@v4` pinned to Node 20 before the deploy step. `action-hosting-deploy@v0` shells out to `npx firebase-tools@latest deploy`, which resolves `node`/`npx` from the runner's system default (Node 24 since 2026-06-16), not from whatever Node version the Actions runtime itself used to execute the action's entrypoint — so `ACTIONS_ALLOW_USE_UNSECURE_NODE_VERSION` does *not* fix this; the PATH itself must be changed. Node 24 has a node-fetch/`http.Agent` regression ([nodejs/node#63989](https://github.com/nodejs/node/issues/63989)) that throws a false-positive "Premature close" on keep-alive socket reuse; firebase-tools retries on that error, but the retried "create release" call is non-idempotent, so it fails with `FAILED_PRECONDITION` ("... is the current active version") even though the original call already succeeded and the site was actually deployed. Remove the `setup-node` step once `action-hosting-deploy`/firebase-tools tolerate Node 24.
