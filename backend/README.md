...
│   │   └── pokemon.py    # GET /pokemon/{id}/resolved
│   ├── models/
...
│   │   └── pokemon_resolved.py  # PokemonResolvedResponse, SmogonSet, EventMove, SpriteUrls
│   ├── services/
...
- `id` — PokéAPI **pokemon** ID (not species ID). Form variants have their own IDs (e.g. Rotom-Wash = 10009).
- `gen` — generation to resolve for (1–9, default 9). Types and base stats reflect gen-accurate values (e.g. Clefairy is Normal-type in gen ≤ 5).
