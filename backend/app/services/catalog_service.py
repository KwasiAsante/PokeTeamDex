...
PokéAPI is the enumeration source — its full name list for each kind is fetched
at startup to determine which entries exist (~900 moves, ~2100 items, ~300
abilities). Each entry is then enriched with Pokémon Showdown data where
available (battle stats, PS flags, display names). PS-only entries that have no
PokéAPI page (Z-moves, Max moves, etc.) are appended afterwards.

preload() is a one-time background task (started at app startup, same pattern as
PokemonResolverService.load_smogon_data()). Until it completes, list_*() methods
raise 503; get_*() methods always work via a live per-request PokéAPI fallback.
"""
...
class CatalogService:
    ...
    async def preload(self) -> None:
        """Enumerate from PokéAPI and merge in PS enrichment for every move/item/ability.
